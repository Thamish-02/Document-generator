...
A file named ccompiler_opt.py.


## Class: _Config

**Description:** An abstract class holds all configurable attributes of `CCompilerOpt`,
...
**Description:** A helper class that provides a collection of fundamental methods
...
**Description:** An abstract class handles caching functionality, provides two
...
**Description:** A helper class for `CCompilerOpt` containing all utilities that
...
**Description:** A helper class for `CCompilerOpt` that managing CPU features.
...
**Description:** A helper class that parsing main arguments of `CCompilerOpt`,
...
**Description:** A helper class for `CCompiler` aims to provide extra build options
to effectively control of compiler optimizations that are directly
related to CPU features.

### Function: new_ccompiler_opt(compiler, dispatch_hpath)

**Description:** Create a new instance of 'CCompilerOpt' and generate the dispatch header
...
### Function: conf_features_partial(self)

**Description:** Return a dictionary of supported CPU features by the platform,
...
### Function: __init__(self, ccompiler)

### Function: dist_compile(self, sources, flags, ccompiler)

**Description:** Wrap CCompiler.compile()

### Function: dist_test(self, source, flags, macros)

**Description:** Return True if 'CCompiler.compile()' able to compile
a source file with certain flags.

### Function: dist_info(self)

**Description:** Return a tuple containing info about (platform, compiler, extra_args),
...
**Description:** Raise a compiler error

### Function: dist_fatal()

**Description:** Raise a distutils error

### Function: dist_log()

**Description:** Print a console message

### Function: dist_load_module(name, path)

**Description:** Load a module from file, required by the abstract class '_Cache'.

### Function: _dist_str()

**Description:** Return a string to print by log and errors.

### Function: _dist_test_spawn_paths(self, cmd, display)

**Description:** Fix msvc SDK ENV path same as distutils do
without it we get c1: fatal error C1356: unable to find mspdbcore.dll

### Function: _dist_test_spawn(cmd, display)

### Function: __init__(self, cache_path)

### Function: __del__(self)

### Function: cache_flush(self)

**Description:** Force update the cache.

### Function: cache_hash(self)

### Function: me(cb)

**Description:** A static method that can be treated as a decorator to
dynamically cache certain methods.

### Function: __init__(self)

### Function: cc_test_flags(self, flags)

**Description:** Returns True if the compiler supports 'flags'.

### Function: cc_test_cexpr(self, cexpr, flags)

**Description:** Same as the above but supports compile-time expressions.

### Function: cc_normalize_flags(self, flags)

**Description:** Remove the conflicts that caused due gathering implied features flags.
...
### Function: _cc_normalize_unix(self, flags)

### Function: _cc_normalize_win(self, flags)

### Function: __init__(self)

### Function: feature_names(self, names, force_flags, macros)

**Description:** Returns a set of CPU feature names that supported by platform and the **C** compiler.
...
### Function: feature_is_exist(self, name)

**Description:** Returns True if a certain feature is exist and covered within
...
### Function: feature_sorted(self, names, reverse)

**Description:** Sort a list of CPU features ordered by the lowest interest.
...
### Function: feature_implies(self, names, keep_origins)

**Description:** Return a set of CPU features that implied by 'names'
...
### Function: feature_implies_c(self, names)

**Description:** same as feature_implies() but combining 'names'

### Function: feature_ahead(self, names)

**Description:** Return list of features in 'names' after remove any
...
### Function: feature_untied(self, names)

**Description:** same as 'feature_ahead()' but if both features implied each other
...
### Function: feature_get_til(self, names, keyisfalse)

**Description:** same as `feature_implies_c()` but stop collecting implied
features when feature's option that provided through
parameter 'keyisfalse' is False, also sorting the returned
features.

### Function: feature_detect(self, names)

**Description:** Return a list of CPU features that required to be detected
sorted from the lowest to highest interest.

### Function: feature_flags(self, names)

**Description:** Return a list of CPU features flags sorted from the lowest
to highest interest.

### Function: feature_test(self, name, force_flags, macros)

**Description:** Test a certain CPU feature against the compiler through its own
check file.
...
### Function: feature_is_supported(self, name, force_flags, macros)

**Description:** Check if a certain CPU feature is supported by the platform and compiler.
...
### Function: feature_can_autovec(self, name)

**Description:** check if the feature can be auto-vectorized by the compiler

### Function: feature_extra_checks(self, name)

**Description:** Return a list of supported extra checks after testing them against
the compiler.
...
### Function: feature_c_preprocessor(self, feature_name, tabs)

**Description:** Generate C preprocessor definitions and include headers of a CPU feature.
...
### Function: __init__(self, cpu_baseline, cpu_dispatch)

### Function: parse_targets(self, source)

**Description:** Fetch and parse configuration statements that required for
...
### Function: _parse_arg_features(self, arg_name, req_features)

### Function: _parse_target_tokens(self, tokens)

### Function: _parse_token_policy(self, token)

**Description:** validate policy token

### Function: _parse_token_group(self, token, has_baseline, final_targets, extra_flags)

**Description:** validate group token

### Function: _parse_multi_target(self, targets)

**Description:** validate multi targets that defined between parentheses()

### Function: _parse_policy_not_keepbase(self, has_baseline, final_targets, extra_flags)

**Description:** skip all baseline features

### Function: _parse_policy_keepsort(self, has_baseline, final_targets, extra_flags)

**Description:** leave a notice that $keep_sort is on

### Function: _parse_policy_not_keepsort(self, has_baseline, final_targets, extra_flags)

**Description:** sorted depend on the highest interest

### Function: _parse_policy_maxopt(self, has_baseline, final_targets, extra_flags)

**Description:** append the compiler optimization flags

### Function: _parse_policy_werror(self, has_baseline, final_targets, extra_flags)

**Description:** force warnings to treated as errors

### Function: _parse_policy_autovec(self, has_baseline, final_targets, extra_flags)

**Description:** skip features that has no auto-vectorized support by compiler

### Function: __init__(self, ccompiler, cpu_baseline, cpu_dispatch, cache_path)

### Function: is_cached(self)

**Description:** Returns True if the class loaded from the cache file

### Function: cpu_baseline_flags(self)

**Description:** Returns a list of final CPU baseline compiler flags

### Function: cpu_baseline_names(self)

**Description:** return a list of final CPU baseline feature names

### Function: cpu_dispatch_names(self)

**Description:** return a list of final CPU dispatch feature names

### Function: try_dispatch(self, sources, src_dir, ccompiler)

**Description:** Compile one or more dispatch-able sources and generates object files,
...
### Function: generate_dispatch_header(self, header_path)

**Description:** Generate the dispatch header which contains the #definitions and headers
...
### Function: report(self, full)

### Function: _wrap_target(self, output_dir, dispatch_src, target, nochange)

### Function: _generate_config(self, output_dir, dispatch_src, targets, has_baseline)

### Function: to_str(arg)

### Function: cache_wrap_me(self)

### Function: ver_flags(f)

### Function: sort_cb(k)

### Function: get_implies(name, _caller)
