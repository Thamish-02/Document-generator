...
A file named _triinterpolate.py.
...
**Description:** Implementation of reduced HCT triangular element with explicit shape
functions.
...
**Description:** Abstract base class for classes used to estimate a function's first
...
**Description:** dz is imposed by user; accounts for scaling if any.

## Class: _DOF_estimator_geom

**Description:** Fast 'geometric' approximation, recommended for large arrays.

## Class: _DOF_estimator_min_E

**Description:** The 'smoothest' approximation, df is computed through global minimization
of the bending energy:
  E(f) = integral[(d2z/dx2 + d2z/dy2 + 2 d2z/dxdy)**2 dA]

## Class: _Sparse_Matrix_coo

### Function: _cg(A, b, x0, tol, maxiter)

**Description:** Use Preconditioned Conjugate Gradient iteration to solve A x = b
...
### Function: _safe_inv22_vectorized(M)

**Description:** Inversion of arrays of (2, 2) matrices, returns 0 for rank-deficient
matrices.

*M* : array of (2, 2) matrices to inverse, shape (n, 2, 2)

### Function: _pseudo_inv22sym_vectorized(M)

**Description:** Inversion of arrays of (2, 2) SYMMETRIC matrices; returns the
...
### Function: _scalar_vectorized(scalar, M)

**Description:** Scalar product between scalars and matrices.

### Function: _transpose_vectorized(M)

**Description:** Transposition of an array of matrices *M*.

### Function: _roll_vectorized(M, roll_indices, axis)

**Description:** Roll an array of matrices along *axis* (0: rows, 1: columns) according to
an array of indices *roll_indices*.

### Function: _to_matrix_vectorized(M)

**Description:** Build an array of matrices from individuals np.arrays of identical shapes.
...
### Function: _extract_submatrices(M, block_indices, block_size, axis)

**Description:** Extract selected blocks of a matrices *M* depending on parameters
...
### Function: __init__(self, triangulation, z, trifinder)

### Function: _interpolate_multikeys(self, x, y, tri_index, return_keys)

**Description:** Versatile (private) method defined for all TriInterpolators.
...
### Function: _interpolate_single_key(self, return_key, tri_index, x, y)

**Description:** Interpolate at points belonging to the triangulation
...
### Function: gradient(self, x, y)

### Function: _interpolate_single_key(self, return_key, tri_index, x, y)

### Function: __init__(self, triangulation, z, kind, trifinder, dz)
...
### Function: _compute_dof(self, kind, dz)

**Description:** Compute and return nodal dofs according to kind.
...
### Function: _get_alpha_vec(x, y, tris_pts)

**Description:** Fast (vectorized) function to compute barycentric coordinates alpha.
...
### Function: _get_jacobian(tris_pts)

**Description:** Fast (vectorized) function to compute triangle jacobian matrix.
...
### Function: _compute_tri_eccentricities(tris_pts)

**Description:** Compute triangle eccentricities.
...
### Function: get_function_values(self, alpha, ecc, dofs)
...
### Function: get_function_derivatives(self, alpha, J, ecc, dofs)
...
### Function: get_function_hessians(self, alpha, J, ecc, dofs)
...
### Function: get_d2Sidksij2(self, alpha, ecc)
...
### Function: get_bending_matrices(self, J, ecc)
...
### Function: get_Hrot_from_J(self, J, return_area)
...
### Function: get_Kff_and_Ff(self, J, ecc, triangles, Uc)

**Description:** Build K and F for the following elliptic formulation:
...
### Function: __init__(self, interpolator)

### Function: compute_dz(self)

### Function: compute_dof_from_df(self)

**Description:** Compute reduced-HCT elements degrees of freedom, from the gradient.

### Function: get_dof_vec(tri_z, tri_dz, J)

**Description:** Compute the dof vector of a triangle, from the value of f, df and
...
### Function: compute_dz(self, dz)

### Function: compute_dz(self)

**Description:** self.df is computed as weighted average of _triangles sharing a common
...
### Function: compute_geom_weights(self)

**Description:** Build the (nelems, 3) weights coeffs of _triangles angles,
renormalized so that np.sum(weights, axis=1) == np.ones(nelems)

### Function: compute_geom_grads(self)

**Description:** Compute the (global) gradient component of f assumed linear (~f).
...
**Description:** Elliptic solver for bending energy minimization.
Uses a dedicated 'toy' sparse Jacobi PCG solver.

### Function: __init__(self, vals, rows, cols, shape)

**Description:** Create a sparse matrix in coo format.
...
**Description:** Dot product of self by a vector *V* in sparse-dense to dense format
*V* dense vector of shape (self.m,).

### Function: compress_csc(self)

**Description:** Compress rows, cols, vals / summing duplicates. Sort for csc format.

### Function: compress_csr(self)

**Description:** Compress rows, cols, vals / summing duplicates. Sort for csr format.

### Function: to_dense(self)

**Description:** Return a dense matrix representing self, mainly for debugging purposes.

### Function: __str__(self)

### Function: diag(self)

**Description:** Return the (dense) vector of the diagonal elements.
