...
A file named _process_common.py.


### Function: read_no_interrupt(p)

**Description:** Read from a pipe ignoring EINTR errors.

This is necessary because when reading from pipes with GUI event loops
running in the background, often interrupts are raised that stop the
command from completing.

### Function: process_handler(cmd, callback, stderr)

**Description:** Open a command in a shell subprocess and execute a callback.
...
### Function: getoutputerror(cmd)

**Description:** Return (standard output, standard error) of executing cmd in a shell.
...
### Function: get_output_error_code(cmd)

**Description:** Return (standard output, standard error, return code) of executing cmd
in a shell.
...
### Function: arg_split(s, posix, strict)
