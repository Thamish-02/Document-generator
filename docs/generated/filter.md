...
A file named filter.py.


## Class: ElementFilter

**Description:** `ElementFilter` encapsulates the logic necessary to decide:
...
**Description:** Each MatchRule encapsulates the logic behind a single argument
passed in to one of the Beautiful Soup find* methods.

## Class: TagNameMatchRule

**Description:** A MatchRule implementing the rules for matches against tag name.

## Class: AttributeValueMatchRule

**Description:** A MatchRule implementing the rules for matches against attribute value.

## Class: StringMatchRule

**Description:** A MatchRule implementing the rules for matches against a NavigableString.

## Class: SoupStrainer

**Description:** The `ElementFilter` subclass used internally by Beautiful Soup.
...
### Function: __init__(self, match_function)

**Description:** Pass in a match function to easily customize the behavior of
...
### Function: includes_everything(self)

**Description:** Does this `ElementFilter` obviously include everything? If so,
...
### Function: excludes_everything(self)

**Description:** Does this `ElementFilter` obviously exclude everything? If
...
### Function: match(self, element, _known_rules)

**Description:** Does the given PageElement match the rules set down by this
ElementFilter?
...
### Function: filter(self, generator)

**Description:** The most generic search method offered by Beautiful Soup.

Acts like Python's built-in `filter`, using
`ElementFilter.match` as the filtering function.

### Function: find(self, generator)

**Description:** A lower-level equivalent of :py:meth:`Tag.find`.
...
### Function: find_all(self, generator, limit)

**Description:** A lower-level equivalent of :py:meth:`Tag.find_all`.
...
### Function: allow_tag_creation(self, nsprefix, name, attrs)

**Description:** Based on the name and attributes of a tag, see whether this
...
### Function: allow_string_creation(self, string)

**Description:** Based on the content of a string, see whether this
...
### Function: __init__(self, string, pattern, function, present, exclude_everything)

### Function: _base_match(self, string)

**Description:** Run the 'cheap' portion of a match, trying to get an answer without
...
### Function: matches_string(self, string)

### Function: __repr__(self)

### Function: __eq__(self, other)

### Function: matches_tag(self, tag)

### Function: __init__(self, name, attrs, string)

### Function: includes_everything(self)

**Description:** Check whether the provided rules will obviously include
everything. (They might include everything even if this returns `False`,
but not in an obvious way.)

### Function: excludes_everything(self)

**Description:** Check whether the provided rules will obviously exclude
...
### Function: _make_match_rules(cls, obj, rule_class)

**Description:** Convert a vaguely-specific 'object' into one or more well-defined
`MatchRule` objects.
...
**Description:** Do the rules of this `SoupStrainer` trigger a match against the
given `Tag`?
...
### Function: _attribute_match(self, attr_value, rules)
...
**Description:** Based on the content of a markup string, see whether this
`SoupStrainer` will allow it to be instantiated as a
`NavigableString` object, or whether it should be ignored.

### Function: matches_any_string_rule(self, string)

**Description:** See whether the content of a string matches any of
...
### Function: search_tag(self, name, attrs)

**Description:** A less elegant version of `allow_tag_creation`. Deprecated as of 4.13.0

### Function: search(self, element)

**Description:** A less elegant version of match(). Deprecated as of 4.13.0.

:meta private:

### Function: _match_attribute_value_helper(attr_values)
