...
A file named loggingTools.py.


## Class: LevelFormatter

**Description:** Log formatter with level-specific formatting.
...
**Description:** A more sophisticated logging system configuation manager.
...
### Function: _resetExistingLoggers(parent)

**Description:** Reset the logger named 'parent' and all its children to their initial
state, if they already exist in the current configuration.

## Class: Timer

**Description:** Keeps track of overall time and split/lap times.
...
**Description:** Provides a hierarchical filter for log entries based on channel names.
...
**Description:** Mixin class that adds logging functionality to another class.
...
### Function: deprecateArgument(name, msg, category)

**Description:** Raise a warning about deprecated function argument 'name'.

### Function: deprecateFunction(msg, category)

**Description:** Decorator to raise a warning when a deprecated function is called.

### Function: __init__(self, fmt, datefmt, style)

### Function: format(self, record)

### Function: __init__(self, logger, msg, level, start)

### Function: reset(self, start)

**Description:** Reset timer to 'start_time' or the current time.

### Function: time(self)

**Description:** Return the overall time (in seconds) since the timer started.

### Function: split(self)

**Description:** Split and return the lap time (in seconds) in between splits.

### Function: formatTime(self, msg, time)

**Description:** Format 'time' value in 'msg' and return formatted string.
...
**Description:** Start a new lap

### Function: __exit__(self, exc_type, exc_value, traceback)

**Description:** End the current lap. If timer has a logger, log the time elapsed,
using the format string in self.msg (or the default one).

### Function: __call__(self, func_or_msg)

**Description:** If the first argument is a function, return a decorator which runs
...
### Function: __init__(self, logger, level)
...
### Function: assertRegex(self, regexp, msg)
