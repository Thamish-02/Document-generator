...
A file named artist.py.


### Function: _prevent_rasterization(draw)

### Function: allow_rasterization(draw)
...
### Function: _finalize_rasterization(draw)

**Description:** Decorator for Artist.draw method. Needed on the outermost artist, i.e.
Figure, to finish up if the render is still in rasterized mode.

### Function: _stale_axes_callback(self, val)
...
### Function: _get_tightbbox_for_layout_only(obj)

**Description:** Matplotlib's `.Axes.get_tightbbox` and `.Axis.get_tightbbox` support a
...
### Function: draw_wrapper(artist, renderer)
...
### Function: _update_set_signature_and_docstring(cls)

**Description:** Update the signature of the set function to list all properties
...
### Function: convert_xunits(self, x)
...
### Function: convert_yunits(self, y)
...
### Function: remove_callback(self, oid)
...
### Function: is_transform_set(self)
...
### Function: _different_canvas(self, event)

**Description:** Check whether an *event* occurred on a canvas other that this artist's canvas.
...
### Function: set_picker(self, picker)
...
### Function: set_path_effects(self, path_effects)
...
### Function: get_path_effects(self)

### Function: get_figure(self, root)

**Description:** Return the `.Figure` or `.SubFigure` instance the artist belongs to.
...
**Description:** Set the `.Figure` or `.SubFigure` instance the artist belongs to.

Parameters
----------
fig : `~matplotlib.figure.Figure` or `~matplotlib.figure.SubFigure`

### Function: set_clip_box(self, clipbox)
...
### Function: _fully_clipped_to_axes(self)

**Description:** Return a boolean flag, ``True`` if the artist is clipped to the Axes
and can thus be skipped in layout calculations. Requires `get_clip_on`
is True, one of `clip_box` or `clip_path` is set, ``clip_box.extents``
is equivalent to ``ax.bbox.extents`` (if set), and ``clip_path._patch``
is equivalent to ``ax.patch`` (if set).

### Function: get_clip_on(self)

**Description:** Return whether the artist uses clipping.

### Function: get_clip_box(self)
...
### Function: get_transformed_clip_path_and_affine(self)

**Description:** Return the clip path with the non-affine part of its
transformation applied, and the remaining affine part of its
transformation.

### Function: set_clip_on(self, b)
...
### Function: _set_gc_clip(self, gc)

**Description:** Set the clip properly for the gc.

### Function: get_rasterized(self)

**Description:** Return whether the artist is to be rasterized.

### Function: set_rasterized(self, rasterized)
...
### Function: get_agg_filter(self)

**Description:** Return filter function to be used for agg filter.

### Function: set_agg_filter(self, filter_func)
...
### Function: _set_alpha_for_array(self, alpha)
...
### Function: set_in_layout(self, in_layout)
...
### Function: get_zorder(self)

**Description:** Return the artist's zorder.

### Function: set_zorder(self, level)
...
### Function: _update_props(self, props, errfmt)

**Description:** Helper for `.Artist.set` and `.Artist.update`.

*errfmt* is used to generate error messages for invalid property
names; it gets formatted with ``type(self)`` for "{cls}" and the
property name for "{prop_name}".

### Function: update(self, props)
...
### Function: _internal_update(self, kwargs)

**Description:** Update artist properties without prenormalizing them, but generating
...
### Function: _cm_set(self)

**Description:** `.Artist.set` context-manager that restores original values at exit.

### Function: findobj(self, match, include_self)
...
### Function: format_cursor_data(self, data)
...
**Description:** Initialize the artist inspector with an `Artist` or an iterable of
`Artist`\s.  If an iterable is used, we assume it is a homogeneous
sequence (all `Artist`\s are of the same type) and it is your
responsibility to make sure this is so.

### Function: get_aliases(self)
...
### Function: get_valid_values(self, attr)
...
### Function: _replace_path(self, source_class)

**Description:** Changes the full path to the public API path that is used
in sphinx. This is needed for links to work.

### Function: get_setters(self)
...
### Function: aliased_name_rest(self, s, target)
...
### Function: pprint_setters(self, prop, leadingspace)
...
### Function: pprint_setters_rest(self, prop, leadingspace)
